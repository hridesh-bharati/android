  ...
  return (
    <View style={styles.container}>
      <View style={styles.card}>
        ...
        <Text style={styles.name}>{user?.email?.split('@')[0] || 'Student'}</Text>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 18, fontWeight: '700', color: '#0f172a', textTransform: 'capitalize' },
  email: { fontSize: 12, color: '#64748b', marginTop: 4, marginBottom: 14 },
  badge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#ecfdf5', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, gap: 4 },
