// src/dashboard/admin/studentManagement/NewAdmissions.jsx
...
export default function NewAdmissions({ navigation }) {
  ...
  const { filteredList, stats } = useMemo(() => {
    ...
    // Sirf "pending" status wale naye admissions filter honge
    const pendingStudents = admissions.filter(s => getActualStatus(s) === "pending");

    counts.all = pendingStudents.length;
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ color: "#64748b", marginTop: 8 }}>Loading new applications...</Text>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 12, paddingBottom: 40 }}>
      ...
      <View style={styles.filterCard}>
        <View style={styles.filterHeaderRow}>
          <Text style={styles.titleText}>New Applications ({filteredList.length})</Text>
        </View>
        ...
        <View style={styles.branchTabsRow}>
          {["all", "Main", "East"].map((b) => {
            ...
            const countKey = b === "all" ? "all" : b.toLowerCase();
            return (
              <TouchableOpacity
                key={b}
              ...
              >
                <Text style={[styles.branchTabText, isSelected && styles.activeBranchTabText]}>
                  {b.toUpperCase()} ({stats[countKey] || 0})
                </Text>
              </TouchableOpacity>
            );
          })}
      ...
      <View style={styles.grid}>
        {filteredList.length > 0 ? (
          ...
          <View style={styles.emptyBox}>
            <MaterialIcons name="inbox" size={40} color="#cbd5e1" style={{ marginBottom: 6 }} />
            <Text style={styles.emptyText}>No pending admission requests.</Text>
          </View>
        )}
      </View>
    </ScrollView>
